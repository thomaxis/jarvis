"""macOS Agent -- connects to cloud brain and executes macOS-specific actions."""
...
from agents.macos.actions import apps, files, system, terminal

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(name)s] %(levelname)s: %(message)s", datefmt="%H:%M:%S")
log = logging.getLogger("jarvis.macos")
    ...
    async def execute_action(self, action: ActionRequest) -> ActionResult:
        ...
        if action_type == "open_app":
            ...
            if target == "lock":
                ...
                success, details = system.sleep_display()
...
def load_config() -> dict:
    ...
    if config_path.exists():
        with open(config_path, "rb") as f:
            ...
            brain = raw.get("brain", {})
            return {
                "device_id": agent.get("device_id", "macos-main"),
                "platform": agent.get("platform", "macos"),
    ...
    return {
        "device_id": os.environ.get("JARVIS_DEVICE_ID", "macos-main"),
        "platform": "macos",
...
async def text_mode(agent: MacOSAgent) -> None:
    print(f"Jarvis macOS Agent ({agent.device_id})")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Jarvis macOS Agent")
    parser.add_argument("--text-only", action="store_true")
    args = parser.parse_args()
    config = load_config()
    agent = MacOSAgent(config)
    if args.text_only:
