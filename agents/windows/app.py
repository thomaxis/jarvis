...
A persistent desktop app with system tray icon and floating chat window.
...
class JarvisApp:
    """Main Jarvis Windows application with GUI and brain connection."""
    ...
    def __init__(self) -> None:
        ...
        # Build GUI
        self._root = tk.Tk()
        self._root.withdraw()  # Start hidden, tray only
        self._build_window()
    ...
    def _build_window(self) -> None:
        ...
        self._root.geometry("420x650")
        self._root.minsize(360, 500)
        ...
        # Welcome message
        self._add_system_message("Jarvis OS v0.1.0 -- Connecting to brain...")

        # --- Input area ---
        input_container = tk.Frame(self._root, bg=BG_PANEL, height=60)
        input_container.pack(fill=tk.X, side=tk.BOTTOM)
        input_container.pack_propagate(False)

        input_inner = tk.Frame(input_container, bg=BG_INPUT, padx=2, pady=2)
    ...
    def _add_message(self, role: str, text: str) -> None:
        ...
        # Outer frame for the message row
        ...
        msg_label = tk.Label(
            ...
            wraplength=350, justify=tk.LEFT, anchor="nw",
        )
    ...
    def _add_system_message(self, text: str) -> None:
        """Add a system status message (centered, dim)."""
        row = tk.Frame(self._chat_frame, bg=BG_DARK)
    ...
    def _on_send(self, event: Any = None) -> None:
        ...
        if self._loop and self._connected:
            ...
            self._add_system_message("Not connected to brain. Retrying...")
    ...
    async def _connect_loop(self) -> None:
        """Connect to brain with auto-reconnect."""
        import websockets
        ...
        while self._running:
            try:
                ...
                async with websockets.connect(
                    self._brain_ws, additional_headers=headers
                ) as ws:
                    self._ws = ws
    ...
    async def _send_to_brain(self, text: str) -> None:
        """Send user input to brain via WebSocket."""
        if self._ws:
            try:
    ...
    async def _handle_message(self, msg: dict) -> None:
        """Handle incoming brain messages."""
        event = msg.get("event", "")
    ...
    async def _execute_action(self, action: dict) -> None:
        """Execute an action from the brain."""
        ...
        status_text = f"[Action] {action_type} {target}: {'OK' if success else 'FAIL'} -- {details}"
